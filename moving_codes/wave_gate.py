...
# Parameters
r = 1  # Base step length
alpha = np.radians(30)  
leg_length = 5  
num_steps = 10  # Number of steps for each leg movement
...
ax.set_xlabel("X")
ax.set_ylabel("Y")
ax.set_title("Hexapod - Wave Gait Animation")
...
move_count = 0  # Counter for total moves to track first 3 moves

def update(frame):
    """Update function for animation."""
    global foot_positions, moving_leg, step, move_count
    
    # Determine step distance based on move count
    step_distance = r if move_count < 3 else 2 * r
    
    # Move the current leg progressively
    if step < num_steps:
        t = step / num_steps  # Normalized progress of the step
        foot_positions[moving_leg, 1] += (step_distance / num_steps)  # Increment leg position gradually
        step += 1
    else:
