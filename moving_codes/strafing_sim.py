...
def znajdz_punkty_rowno_odlegle_na_paraboli(r, h, ilosc_punktow_na_krzywej, ilosc_probek, bufor_x):
    L = dlugosc_funkcji_ruchu_nogi(r, h, ilosc_probek)  # Obliczenie długości trajektorii
    dlugosc_kroku = L / ilosc_punktow_na_krzywej  # Obliczenie odległości między punktami
    suma = 0
    punkty = []
    
    for i in range(1, ilosc_probek):
        z_0 = funkcja_ruchu_nogi(r, h, (i - 1) / ilosc_probek * r)  # Poprzedni punkt Z
        z_1 = funkcja_ruchu_nogi(r, h, i / ilosc_probek * r)  # Aktualny punkt Z
        dlugosc = np.sqrt((z_1 - z_0) ** 2 + (r / ilosc_probek) ** 2)  # Długość odcinka

        suma += dlugosc
        if suma > dlugosc_kroku:  # Jeśli przekroczyliśmy odległość kroku, dodajemy punkt
            suma -= dlugosc_kroku
            punkty.append([i / ilosc_probek * r + bufor_x, 0, z_1])  # Przesunięcie w X, zamiast Y

        if len(punkty) == ilosc_punktow_na_krzywej - 1:  # Jeśli mamy wystarczającą liczbę punktów, przerywamy
            break

    punkty.append([bufor_x + r, 0, 0])  # Ostatni punkt na trajektorii
    return punkty
...
punkty_etap2_ruchu_x = np.linspace(r * (ilosc_punktow_na_krzywych - 1) / ilosc_punktow_na_krzywych, 0, ilosc_punktow_na_krzywych)
punkty_etap2_ruchu = [[punkty_etap2_ruchu_x[i], 0, 0] for i in range(ilosc_punktow_na_krzywych)]
punkty_etap3_ruchu_x = np.linspace(-r / ilosc_punktow_na_krzywych, -r, ilosc_punktow_na_krzywych)
punkty_etap3_ruchu = [[punkty_etap3_ruchu_x[i], 0, 0] for i in range(ilosc_punktow_na_krzywych)]
