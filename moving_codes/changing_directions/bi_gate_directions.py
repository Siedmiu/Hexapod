...
class BiGateHexapodController:
    def __init__(self):
        ...
        print("Bi-Gate Hexapod Ready! Use arrow keys to control:")
        print("↑ - Forward")
        print("↓ - Backward") 
        print("→ - Right")
        print("← - Left")
    ...
    def setup_hexapod_parameters(self):
        ...
        self.h = self.l3 / 4
        self.r = self.h
        self.ilosc_punktow_na_krzywych = 10
    ...
    def generate_base_trajectories(self):
        """Generate base trajectory segments for all directions"""
        ...
        for direction in [self.FORWARD, self.BACKWARD, self.RIGHT, self.LEFT]:
            ...
            punkty_etap1_ruchu = self.znajdz_punkty_rowno_odlegle_na_paraboli(
                self.r, self.h / 2, self.ilosc_punktow_na_krzywych, 10000, 0, ruch_strafing)
            
            if ruch_strafing:
                punkty_etap2_ruchu_coord = np.linspace(
                    self.r * (self.ilosc_punktow_na_krzywych - 1) / self.ilosc_punktow_na_krzywych, 0, self.ilosc_punktow_na_krzywych)
                punkty_etap2_ruchu = [[punkty_etap2_ruchu_coord[i], 0, 0] for i in range(self.ilosc_punktow_na_krzywych)]
                punkty_etap3_ruchu_coord = np.linspace(-self.r / self.ilosc_punktow_na_krzywych, -self.r, self.ilosc_punktow_na_krzywych)
                punkty_etap3_ruchu = [[punkty_etap3_ruchu_coord[i], 0, 0] for i in range(self.ilosc_punktow_na_krzywych)]
            else:
                punkty_etap2_ruchu_coord = np.linspace(
                    self.r * (self.ilosc_punktow_na_krzywych - 1) / self.ilosc_punktow_na_krzywych, 0, self.ilosc_punktow_na_krzywych)
                punkty_etap2_ruchu = [[0, punkty_etap2_ruchu_coord[i], 0] for i in range(self.ilosc_punktow_na_krzywych)]
                punkty_etap3_ruchu_coord = np.linspace(-self.r / self.ilosc_punktow_na_krzywych, -self.r, self.ilosc_punktow_na_krzywych)
                punkty_etap3_ruchu = [[0, punkty_etap3_ruchu_coord[i], 0] for i in range(self.ilosc_punktow_na_krzywych)]
            
            punkty_etap4_ruchu = self.znajdz_punkty_rowno_odlegle_na_paraboli(
                2 * self.r, self.h, self.ilosc_punktow_na_krzywych, 20000, -self.r, ruch_strafing)
            punkty_etap5_ruchu = self.znajdz_punkty_rowno_odlegle_na_paraboli(
                self.r, self.h / 2, self.ilosc_punktow_na_krzywych, 10000, -self.r, ruch_strafing)
            
            # Bi-gate specific trajectories
    ...
    def generate_trajectory_sequence(self, direction, phase_type):
        """Generate trajectory sequence for given direction and phase"""
        traj = self.base_trajectories[direction]
        
        # Create stationary sequence for legs 2&5 during startup
        pierwszy_krok_nog_2_5 = []
        for i in range(self.ilosc_punktow_na_krzywych):
            pierwszy_krok_nog_2_5.append([0, 0, 0])
        
        # Build sequence based on phase type
        if phase_type == "startup":
            if traj['ruch_do_tylu'] or traj['ruch_w_lewo']:
                # For reversed directions, startup is actually shutdown of opposite direction
            ...
            if traj['ruch_do_tylu'] or traj['ruch_w_lewo']:
                # For reversed directions, shutdown is actually startup of opposite direction
                cykl_nog_1_4 = traj['etap6'].copy()
                cykl_nog_2_5 = pierwszy_krok_nog_2_5.copy()
                cykl_nog_3_6 = traj['etap1'].copy()
            else:
        ...
        def pad_cycle(cycle, target_length):
            if len(cycle) < target_length:
                # Repeat the last position to extend the cycle
    ...
    def update_animation_queue(self):
        ...
        if self.current_phase == "stopped":
            if self.requested_direction != self.STOP:
                ...
                print(f"Starting bi-gate movement from {old_direction} to direction: {self.current_direction}")
            ...
            if len(self.animation_queue) == 0:
                ...
                print(f"Entering bi-gate main loop for direction: {self.current_direction}")
            ...
            if len(self.animation_queue) == 0:
                ...
                if self.transition_pending:
                    # Direction change or stop requested - begin shutdown for CURRENT direction
                    ...
                    print(f"Bi-gate main loop cycle {self.main_loop_counter} completed, beginning shutdown from CURRENT direction: {self.current_direction}, transitioning to: {self.requested_direction}")
                else:
                    ...
                    if self.main_loop_counter % 5 == 0:  # Print every 5 cycles to avoid spam
                        print(f"Continuing bi-gate main loop, cycle: {self.main_loop_counter}")
                    
        elif self.current_phase == "shutdown":
            if len(self.animation_queue) == 0:
                if self.requested_direction == self.STOP:
                    # Stop requested - return to stopped state
                    old_direction = self.current_direction
                    self.current_direction = self.STOP
                    self.current_phase = "stopped"
                    self.transition_pending = False
                    print(f"Bi-gate hexapod stopped (was moving: {old_direction})")
                    ...
                    for frame in range(startup_traj.shape[1]):
                        self.animation_queue.append(('startup', frame, startup_traj[:, frame]))
                    print(f"Bi-gate transition complete: shutdown({old_direction}) -> startup({self.current_direction})")
                else:
                    ...
                    self.current_phase = "stopped" 
                    self.transition_pending = False
                    print(f"Bi-gate hexapod stopped (fallback, was: {old_direction})")
...
def update_animation(frame, hexapod, lines, status_text):
    ...
    status_info = f"""BI-GATE HEXAPOD STATUS
...
# Main execution
def main():
    # Create bi-gate hexapod controller
    hexapod = BiGateHexapodController()
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Interactive Bi-Gate Hexapod Simulation\nArrow Keys: Move | SPACE: Stop')
