...
class HexapodController:
    def __init__(self):
        ...
        print("Hexapod Ready! Use arrow keys to control:")
        print("↑ - Forward")
        print("↓ - Backward") 
        print("→ - Right")
        print("← - Left")
    ...
    def setup_hexapod_parameters(self):
        ...
        self.h = self.l3 / 4
        self.r = self.h
        self.ilosc_punktow_na_krzywych = 20
    ...
    def generate_base_trajectories(self):
        """Generate base trajectory segments (etap1-5) for all directions"""
        ...
        for direction in [self.FORWARD, self.BACKWARD, self.RIGHT, self.LEFT]:
            ...
            # Generate trajectory segments exactly like in original code
            ...
            punkty_etap4_ruchu = self.znajdz_punkty_rowno_odlegle_na_paraboli(
                2 * self.r, self.h, 2 * self.ilosc_punktow_na_krzywych, 20000, -self.r, ruch_strafing)
    ...
    def generate_trajectory_sequence(self, direction, phase_type):
        ...
        if phase_type == "startup":
            ...
            # Main loop is the same regardless of direction
    ...
    def update_animation_queue(self):
        ...
        if self.current_phase == "stopped":
            if self.requested_direction != self.STOP:
                ...
                old_direction = self.current_direction  # Store for debugging
                ...
                print(f"Starting movement from {old_direction} to direction: {self.current_direction}")
            ...
            if len(self.animation_queue) == 0:
                ...
                print(f"Entering main loop for direction: {self.current_direction}")
            ...
            if len(self.animation_queue) == 0:
                ...
                if self.transition_pending:
                    ...
                    print(f"Main loop cycle {self.main_loop_counter} completed, beginning shutdown from CURRENT direction: {self.current_direction}, transitioning to: {self.requested_direction}")
                else:
                    ...
                    if self.main_loop_counter % 5 == 0:  # Print every 5 cycles to avoid spam
                        print(f"Continuing main loop, cycle: {self.main_loop_counter}")
            ...
            if len(self.animation_queue) == 0:
                if self.requested_direction == self.STOP:
                    ...
                    self.transition_pending = False
                    print(f"Hexapod stopped (was moving: {old_direction})")
                    ...
                    for frame in range(startup_traj.shape[1]):
                        self.animation_queue.append(('startup', frame, startup_traj[:, frame]))
                    print(f"Transition complete: shutdown({old_direction}) -> startup({self.current_direction})")
                else:
                    ...
                    self.current_phase = "stopped" 
                    self.transition_pending = False
                    print(f"Hexapod stopped (fallback, was: {old_direction})")
...
def main():
    ...
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    ax.set_title('Interactive Hexapod Simulation\nArrow Keys: Move | SPACE: Stop')
