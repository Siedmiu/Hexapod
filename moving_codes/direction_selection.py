...
# Pytanie o typ ruchu
print("Wybierz kierunek ruchu hexapoda:")
print("1 - Ruch do przodu")
print("2 - Ruch do tyłu") 
print("3 - Ruch w prawo")
print("4 - Ruch w lewo")
while True:
    wybor = input("Wprowadź 1, 2, 3 lub 4: ").strip()
    if wybor in ['1', '2', '3', '4']:
        kierunek_ruchu = int(wybor)
        break
    else:
        print("Nieprawidłowy wybór. Wprowadź 1, 2, 3 lub 4.")

# Określenie typu ruchu i kierunku
ruch_strafing = kierunek_ruchu in [3, 4]  # 3=prawo, 4=lewo
ruch_do_tylu = kierunek_ruchu == 2
ruch_w_lewo = kierunek_ruchu == 4

nazwy_kierunkow = {1: "do przodu", 2: "do tyłu", 3: "w prawo", 4: "w lewo"}
print(f"Wybrano ruch: {nazwy_kierunkow[kierunek_ruchu]}")
...
def funkcja_ruchu_nogi(r, h, punkt_ruchu): # punkt_ruchu to x dla strafingu, y dla ruchu do przodu
...
def znajdz_punkty_rowno_odlegle_na_paraboli(r, h, ilosc_punktow_na_krzywej, ilosc_probek, bufor, ruch_strafing):
    ...
    for i in range(1,ilosc_probek):
        ...
        if(suma > dlugosc_kroku):
            ...
            if ruch_strafing:
                punkty.append([i/ilosc_probek * r + bufor, 0, z_1])  # Strafing: przesunięcie w X
            else:
                punkty.append([0, i/ilosc_probek * r + bufor, z_1])  # Do przodu: przesunięcie w Y
        if(len(punkty) == ilosc_punktow_na_krzywej - 1):
            break
    
    if ruch_strafing:
        punkty.append([bufor + r, 0, 0])  # Strafing: przesunięcie w X
    else:
        punkty.append([0, bufor + r, 0])  # Do przodu: przesunięcie w Y
    return punkty
...
# Generowanie punktów w zależności od typu ruchu
punkty_etap1_ruchu = znajdz_punkty_rowno_odlegle_na_paraboli(r, h / 2, ilosc_punktow_na_krzywych, 10000, 0, ruch_strafing)

if ruch_strafing:
    # Ruch strafingowy (w kierunku X)
    punkty_etap2_ruchu_coord = np.linspace(r * (ilosc_punktow_na_krzywych - 1) / ilosc_punktow_na_krzywych, 0, ilosc_punktow_na_krzywych)
    punkty_etap2_ruchu = [[punkty_etap2_ruchu_coord[i], 0, 0] for i in range(ilosc_punktow_na_krzywych)]
    punkty_etap3_ruchu_coord = np.linspace(-r / ilosc_punktow_na_krzywych, -r, ilosc_punktow_na_krzywych)
    punkty_etap3_ruchu = [[punkty_etap3_ruchu_coord[i], 0, 0] for i in range(ilosc_punktow_na_krzywych)]
else:
    # Ruch do przodu (w kierunku Y)
    punkty_etap2_ruchu_coord = np.linspace(r * (ilosc_punktow_na_krzywych - 1) / ilosc_punktow_na_krzywych, 0, ilosc_punktow_na_krzywych)
    punkty_etap2_ruchu = [[0, punkty_etap2_ruchu_coord[i], 0] for i in range(ilosc_punktow_na_krzywych)]
    punkty_etap3_ruchu_coord = np.linspace(-r / ilosc_punktow_na_krzywych, -r, ilosc_punktow_na_krzywych)
    punkty_etap3_ruchu = [[0, punkty_etap3_ruchu_coord[i], 0] for i in range(ilosc_punktow_na_krzywych)]

punkty_etap4_ruchu = znajdz_punkty_rowno_odlegle_na_paraboli(2 * r, h, 2 * ilosc_punktow_na_krzywych, 20000, -r, ruch_strafing)
punkty_etap5_ruchu = znajdz_punkty_rowno_odlegle_na_paraboli(r, h / 2, ilosc_punktow_na_krzywych, 10000, -r, ruch_strafing)
...
# Odwrócenie cykli dla ruchu do tyłu lub w lewo
if ruch_do_tylu or ruch_w_lewo:
    print("Odwracanie cykli dla ruchu do tyłu/w lewo...")
...
#RUCH DO TYŁU lub W LEWO - odwracanie cykli:
# Ten kod zostanie wykonany automatycznie powyżej w zależności od wyboru

# Transformacja cykli nóg w zależności od typu ruchu
if ruch_strafing:
    # Transformacja dla ruchu strafingowego (jak w strafing_sim_stare.py)
    cykle_nog = np.array([
        [
            [cykl_ogolny_nog_1_3_5[i][0] * np.cos(nachylenia_nog_do_bokow_platformy_pajaka[j]) +
             cykl_ogolny_nog_1_3_5[i][1] * np.sin(nachylenia_nog_do_bokow_platformy_pajaka[j]),

             -cykl_ogolny_nog_1_3_5[i][0] * np.sin(nachylenia_nog_do_bokow_platformy_pajaka[j]) +
        ...
        ] if j in (0, 2, 4) else  # Nogi 1,3,5 dla strafingu
        [
            [cykl_ogolny_nog_2_4_6[i][0] * np.cos(nachylenia_nog_do_bokow_platformy_pajaka[j]) +
             cykl_ogolny_nog_2_4_6[i][1] * np.sin(nachylenia_nog_do_bokow_platformy_pajaka[j]),

             -cykl_ogolny_nog_2_4_6[i][0] * np.sin(nachylenia_nog_do_bokow_platformy_pajaka[j]) +
    ...
    # Transformacja dla ruchu do przodu (jak w tri_gate_sim_stare.py)
    cykle_nog = np.array([
        [
            [cykl_ogolny_nog_1_3_5[i][1] * np.sin(nachylenia_nog_do_bokow_platformy_pajaka[j]),
             cykl_ogolny_nog_1_3_5[i][1] * np.cos(nachylenia_nog_do_bokow_platformy_pajaka[j]),
...
print(f"Rozpoczynanie symulacji ruchu {nazwy_kierunkow[kierunek_ruchu]}...")
print(f"Pozycje stopy nogi 1 podczas cyklu (pierwsze 5 punktów):")
print(polozenia_stop_podczas_cyklu[1][:5])
