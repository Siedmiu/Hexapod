...
class ArmController(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().warn(f"UART connection failed: {e}")
    ...
    def map_rad_to_servo(self, angle_rad):
        """Mapuje kąt w radianach [-π, π] na serwo [0, 180]"""
        angle_deg = np.degrees(angle_rad)
        return int(angle_deg + 90)
    
    def send_uart_command(self, joint_angles_rad):
        if self.serial_port is None:
            self.get_logger().warn("UART not available. Skipping UART command.")
            return

        for i, angle_rad in enumerate(joint_angles_rad):
            angle_deg = self.map_rad_to_servo(angle_rad)
            message = f"servo{i+1} {int(angle_deg)}\n"
            self.serial_port.write(message.encode('utf-8'))
            self.get_logger().info(f"Sent to UART: {message.strip()}")
        
        # for i, angle_rad in enumerate(joint_angles_rad):
        #     angle_deg = self.map_rad_to_servo(angle_rad)
        #     message = f"servo{i+1} {angle_deg}"
        #     self.get_logger().info(f"[UART SIM] {message}")
    ...
    def move_to_position(self, joint_positions, execution_time=3.0):
        ...
        self.send_uart_command(list(joint_positions.values()))
...
def main(args=None):
    ...
    # Wszystkie stawy w pozycji zerowej
    home_position = {
        'joint1': wychyly_serw_podczas_ruchu[0][0],  # około 28.6 stopni
        'joint2': wychyly_serw_podczas_ruchu[0][1],  # około -40.1 stopni
        'joint3': wychyly_serw_podczas_ruchu[0][2] + np.deg2rad(45)  # około 17.2 stopni
    }
    ...
    try:
        ...
        for i in range(len(wychyly_serw_podczas_ruchu)):
            ...
            # Teraz do alternatywnej pozycji
            controller.move_to_position(alt_position)
            controller.get_logger().info("Sent command to move to alternative position")

            alt_position = {
                'joint1': wychyly_serw_podczas_ruchu[i][0],  # około 28.6 stopni
                'joint2': wychyly_serw_podczas_ruchu[i][1],  # około -40.1 stopni
                'joint3': wychyly_serw_podczas_ruchu[i][2] + np.deg2rad(45)  # około 17.2 stopni
            }
        ...
        # Można też użyć interfejsu akcji
        # controller.execute_trajectory_action(home_position)

        # Spin aby nasłuchiwać informacji zwrotnych (jeśli używasz akcji)
