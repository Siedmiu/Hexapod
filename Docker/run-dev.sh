...
XAUTH_FILE=$(mktemp /tmp/.docker.xauth.import-dev.XXXXXX)
...
if lspci | grep -iq nvidia; then
    ...
    if [ -d "/dev/dri" ]; then
        echo "Found /dev/dri. Trying to mount. This may work for some open-source drivers."
        DOCKER_GPU_ARGS="--device=/dev/dri --group-add video"
    fi
fi

# Add check if Hexapod directory exists
if [ ! -d "$PWD/Hexapod-ROS2-System" ]; then
  echo "Hexapod-ROS2-System directory not found. Cloning repository..."
...
# Pulling development image from Docker Hub
...
# Build workspace in the container if install/setup.bash does not exist
if [ ! -f "$PWD/Hexapod-ROS2-System/sim_and_real/ros2_ws_hex/install/setup.bash" ]; then
  echo "Building workspace in the container..."
  sudo docker run --rm \
  ...
  # Fix permissions after building in the container
  sudo chown -R $USER:$USER "$PWD/Hexapod-ROS2-System"
fi

# Check if user wants diagnostic mode
if [ "$1" == "diagnose" ]; then
  echo "Starting container in diagnostic mode..."
  sudo docker run -it --rm \
    ...
    -v "$PWD/Hexapod-ROS2-System:/ros_ws/Hexapod-ROS2-System" \
    hexapod-dev \
    bash -c "cd /ros_ws/Hexapod-ROS2-System && if [ -f Docker/check-gpu.sh ]; then Docker/check-gpu.sh; else echo 'Diagnostic script does not exist'; fi && source /opt/ros/jazzy/setup.bash && bash"
  exit 0
fi
